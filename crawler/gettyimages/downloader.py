# -*- coding: utf-8 -*-
"""
Created on Sat Jan 20 21:39:49 2024
...
class download_images():
    def __init__(self,api_key,api_secret,username,password,image_id,directory, search_response):
    ...
    def _get_resource_owner_token(self):
        """Get an access token using resource owner grant"""
        url = "https://api.gettyimages.com/v4/oauth2/token"
        payload = f"grant_type=password&client_id={self.api_key}&client_secret={self.api_secret}&username={self.user}&password={self.passwd}"
        headers = {'content-type': 'application/x-www-form-urlencoded'}
        response = requests.request("POST", url, data=payload, headers=headers)
        auth = json.loads(response.content)
        return auth
    
    def _get_download_url(id, auth):
